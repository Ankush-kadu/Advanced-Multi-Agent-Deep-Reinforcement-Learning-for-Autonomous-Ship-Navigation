...
class Vessel():
    ...
    def step(self, delta_c, n_c =115.6/60):  
        ...
        if np.isnan(n_c):
            n_c = 0
        # print(f"delta_c ={delta_c} and n_c ={n_c}")
...
#
        if self.U_des is not None:
            new_state[0:3] = new_state[0:3] 
            new_state[3:6] = new_state[3:6] 
            new_state[6:9] = new_state[6:9]

            new_state[14] = new_state[14] 

            new_state_der[0:3] = new_state_der[0:3] 
            new_state_der[3:6] = new_state_der[3:6] 
            new_state_der[6:9] = new_state_der[6:9] 
            
            new_state_der[14] = new_state_der[14] 
...
#
    ...
    def inti_state(self):
        num = 2
        region_width = 100 // num
        start_x = np.random.rand() * region_width + num * region_width
        ...
        initial_state = np.array([
                1, 0, 0, 0, 0, 0, start_x, start_y, 0, 0, 0, 0,
                np.random.uniform(-np.pi, np.pi), 0,
                115.5 / 60    # 15 values
            ], dtype=np.float64)
    ...
    def pf_calculate(self):
        ...
        x_init, y_init, x_goal, y_goal = self.startx, self.starty, self.goalx, self.goaly
        x, y, psi, u, v, r = self.state[6], self.state[7], self.state[12], self.state[0], self.state[1], self.state[5]
